...
function getPicture(req,res,next){
    ...
    courseService.getPicture(req.params.id).then(courses => {console.log('# of Courses sent:', courses.length);
        res.json(courses)}).catch(err => next(err));
}
function getArtistPictures(req,res,next){
    console.log('THe params are ',req.params);
    courseService.getArtistPictures(req.params.artist).then(courses => {console.log('# of Courses sent:', courses.length);
        res.json(courses)}).catch(err => next(err));
}
